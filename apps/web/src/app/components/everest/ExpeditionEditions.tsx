      ...
      <div className="max-w-[1440px] mx-auto px-8 flex flex-col gap-24">
        <div className="flex flex-col gap-8">
          ...
          <h3 className="font-['Radley'] font-light text-[44px] md:text-[56px] leading-[1.1] max-w-[32ch]">
            {items.length === 1
              ? "One way to read this mountain."
              : `${items.length > 1 ? `${items.length} ways` : "Ways"} to read this mountain.`}
          </h3>
        </div>
        ...
        <div className="flex flex-col border-t border-white/20">
          {items.map((ed, idx) => (
            <div
              key={idx}
              className="flex flex-col md:flex-row gap-8 md:gap-12 py-12 border-b border-white/20 items-start md:items-center hover:bg-white/[0.02] transition-colors cursor-pointer group"
            >
