...
export function RouteMap({ waypoints, routePhilosophy, acclimatisationNote, summitWindowNote }: Props) {
      ...
      <div className="max-w-[1440px] mx-auto px-8 flex flex-col gap-24">
        <div className="flex flex-col gap-8">
          ...
          <h3 className="font-['Radley'] font-light text-[44px] md:text-[56px] leading-[1.1] max-w-[32ch]">
            {points.length > 0 ? `From ${points[0].name} to the summit. ${points.length - 1} points on the line.` : 'The route.'}
          </h3>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 border-t border-white/20 pt-16">
          <div className="flex flex-col gap-4">
            ...
            {routePhilosophy && (
              <p className="font-['Lexend'] font-light text-[#C8CDD2] text-[15px] leading-[1.8] max-w-[40ch]">{routePhilosophy}</p>
            )}
          </div>
          <div className="flex flex-col gap-4">
            <span className="font-['JetBrains_Mono'] uppercase tracking-[0.22em] text-[11px] text-[#C8CDD2]">ACCLIMATISATION CYCLE</span>
            {acclimatisationNote && (
              <p className="font-['Lexend'] font-light text-[#C8CDD2] text-[15px] leading-[1.8] max-w-[40ch]">{acclimatisationNote}</p>
            )}
          </div>
          <div className="flex flex-col gap-4">
            <span className="font-['JetBrains_Mono'] uppercase tracking-[0.22em] text-[11px] text-[#C8CDD2]">SUMMIT WINDOW</span>
            {summitWindowNote && (
              <p className="font-['Lexend'] font-light text-[#C8CDD2] text-[15px] leading-[1.8] max-w-[40ch]">{summitWindowNote}</p>
            )}
          </div>
        </div>
      </div>
