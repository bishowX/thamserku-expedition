...
export function ExpeditionClosing({ name, closingImage, slug }: Props) {
  const imageSrc = closingImage
    ? urlFor(closingImage as SanityImageSource)
        .width(1920)
        .url()
    : null;
  ...
  return (
    <section
      ref={sectionRef}
      id="configure"
      className="relative bg-[#1A1A1A] w-full flex items-center justify-center text-center px-5 md:px-8 py-24 md:py-32 overflow-hidden scroll-mt-28"
    >
      ...
      <div className="relative z-10 flex flex-col items-center gap-6 max-w-[640px] w-full">
        ...
        <p
          data-reveal
          className="font-['DM_Sans'] font-light text-body leading-[1.6] text-[#C8CDD2] max-w-[480px]"
        >
          Select your Edition, customize add-ons, and receive a detailed
          proposal with investment guidance — sent privately to your email
          within 60 seconds.
        </p>
        ...
        <span
          data-reveal
          className="mt-2 font-['DM_Mono'] uppercase tracking-[0.22em] text-[11px] text-[#C8CDD2]"
        >
