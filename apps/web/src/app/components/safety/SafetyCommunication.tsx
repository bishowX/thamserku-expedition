    ...
    <section className="bg-white section-padding">
      <div className="max-w-[1440px] mx-auto flex flex-col gap-6">
        <div className="flex flex-col gap-4">
          ...
          <h2 className="font-['Radley'] text-fluid-heading text-[#1A1A1A] leading-[1.2]">
        ...
        <div className="border-t border-[#1A1A1A]/10 pt-10 md:pt-6 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10 md:gap-12">
          {items.map((item, i) => (
            <div key={i} className="flex flex-col gap-5 md:gap-6">
              <h3 className="font-['Radley'] text-[24px] text-[#1A1A1A] leading-[1.3]">
                {item.title}
              </h3>
              <p className="font-['Lexend'] font-light text-[15px] text-[#5A6673] leading-[1.6]">
                {item.body}
              </p>
            </div>
          ))}
