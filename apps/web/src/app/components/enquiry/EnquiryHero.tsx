...
export const EnquiryHero = ({ data }: { data?: ConsultationPage }) => {
  ...
  const metaResponse = data?.heroMetaResponse;
  const metaHandledBy = data?.heroMetaHandledBy;
  const metaLanguages = data?.heroMetaLanguages;
  const metaConfidentiality = data?.heroMetaConfidentiality;
        ...
        <div className="max-w-[1440px] mx-auto px-8 py-6">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 md:gap-0 divide-y md:divide-y-0 md:divide-x divide-[#2E353C]">
            <div className="font-['JetBrains_Mono'] uppercase tracking-[0.22em] text-[10px] text-[#C8CDD2] md:pr-6 py-2 md:py-0">
              RESPONSE <span className="mx-2 text-[#5A6673]">·</span> {metaResponse}
            </div>
            <div className="font-['JetBrains_Mono'] uppercase tracking-[0.22em] text-[10px] text-[#C8CDD2] md:px-6 py-2 md:py-0">
              HANDLED BY <span className="mx-2 text-[#5A6673]">·</span> {metaHandledBy}
            </div>
            <div className="font-['JetBrains_Mono'] uppercase tracking-[0.22em] text-[10px] text-[#C8CDD2] md:px-6 py-2 md:py-0">
              LANGUAGES <span className="mx-2 text-[#5A6673]">·</span> {metaLanguages}
            </div>
            <div className="font-['JetBrains_Mono'] uppercase tracking-[0.22em] text-[10px] text-[#C8CDD2] md:pl-6 py-2 md:py-0">
              CONFIDENTIALITY <span className="mx-2 text-[#5A6673]">·</span> {metaConfidentiality}
            </div>
          </div>
        </div>
      </div>
