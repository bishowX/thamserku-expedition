...
import type { PathwayFaqItem } from "../../../lib/queries";
...
export function PathwayFAQ({ page }: { page?: PageData }) {
  const eyebrow = page?.faqEyebrow;
  const headline = page?.faqHeadline;
  const subheading = page?.faqSubheading;
  const ctaLabel = page?.faqCtaLabel;
  ...
  if (!eyebrow || !headline || faqs.length === 0) return null;
      ...
      <div className="relative z-10 w-full max-w-[880px] mx-auto px-8 flex flex-col items-center">
        ...
        <div className="w-full flex flex-col mb-10 md:mb-24 border-b border-[#C8CDD2]/30">
          {faqs.map((faq, idx) => {
            ...
            return (
              <div key={faq._key} className="flex flex-col border-t border-[#C8CDD2]/30">
                <button
                  ...
                  aria-controls={`faq-answer-pathway-${idx}`}
                  id={`faq-question-pathway-${idx}`}
                >
                  ...
                  <span className={`font-['JetBrains_Mono'] text-[14px] text-[#C8CDD2] group-hover:text-white transition-all duration-[250ms] ease-out transform ${isOpen ? "rotate-180" : "rotate-0"}`}>
                    ▾
                  </span>
                </button>
                <div
                  id={`faq-answer-pathway-${idx}`}
                  role="region"
                  aria-labelledby={`faq-question-pathway-${idx}`}
