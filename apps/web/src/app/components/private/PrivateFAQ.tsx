...
type Props = { page?: FaqData };

export function PrivateFAQ({ page }: Props) {
  const eyebrow = page?.faqEyebrow ?? "FREQUENTLY ASKED — PRIVATE EXPEDITIONS";
  const headline = page?.faqHeadline ?? "Five quiet answers, before you write to us.";
  const subheading = page?.faqSubheading ?? "Most clients arrive at private expeditions with the same questions. These are the most common.";
  const ctaLabel = page?.faqCtaLabel ?? "READ ALL FAQS ON THE MAIN FAQ PAGE";
            ...
            return (
              <div key={faq._key} className="flex flex-col border-t border-[#C8CDD2]/30">
                <button
                  ...
                  aria-controls={`faq-answer-private-${idx}`}
                  id={`faq-question-private-${idx}`}
                >
                ...
                <div
                  id={`faq-answer-private-${idx}`}
                  role="region"
                  aria-labelledby={`faq-question-private-${idx}`}
